...
{
	//Gathering fragment shader information within the shader folder (Both BasicColor.vs and BasicColorInvert.fs)
...
void AppClass::InitVariables(void)
{
	...
	lVertex.push_back(glm::vec3(1.0f, 0.0f, 0.0f)); //color red
	//vertex 2
	lVertex.push_back(glm::vec3(1.0f, -1.0f, 0.0f)); //position
	lVertex.push_back(glm::vec3(0.0f, 1.0f, 0.0f)); //color green
	//vertex 3
	lVertex.push_back(glm::vec3(0.0f, 1.0f, 0.0f)); //position
	lVertex.push_back(glm::vec3(0.0f, 0.0f, 1.0f)); //color blue
...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	if (a_event.key.code == sf::Keyboard::Key::Escape)//Event says I pressed the Escape key
		...
		m_v3Color = glm::vec3(1.0f, 0.0f, 0.0f); //red
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Num2))
		m_v3Color = glm::vec3(0.0f, 1.0f, 0.0f); //green
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Num3))
		m_v3Color = glm::vec3(0.0f, 0.0f, 1.0f); //blue
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Num0))
		m_v3Color = glm::vec3(-1.0f, -1.0f, -1.0f); //Back to normal color split

	//If the letter c is pressed, change to complimentary color. If already pressed disable complementary color
	if (a_event.key.code == sf::Keyboard::Key::C)
	{
		m_ComplimentaryColor = !m_ComplimentaryColor;
	}
}
void AppClass::Display(void)
{
	...
	//read uniforms and send values for solid color
	...
	//read uniforms and send values for complementary color
	GLuint ComplimentaryColor = glGetUniformLocation(m_uShaderProgramID, "Complimentary");
	glUniform1i(ComplimentaryColor, m_ComplimentaryColor);
